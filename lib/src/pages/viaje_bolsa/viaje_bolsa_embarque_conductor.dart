...
class ViajeBolsaEmbarquePage_Conductor extends StatefulWidget {
  const ViajeBolsaEmbarquePage_Conductor({Key? key}) : super(key: key);

  @override
  State<ViajeBolsaEmbarquePage_Conductor> createState() => _ViajeBolsaEmbarquePage_ConductorState();
}

class _ViajeBolsaEmbarquePage_ConductorState extends State<ViajeBolsaEmbarquePage_Conductor> {
  ...
  void initState() {
    ...
    // _init();
    super.initState();
    Log.ingreso(context, "CONDUCTOR: INGRESO A EMBARQUE PASAJEROS ");
    _showDialog();
  }
    ...
    return WillPopScope(
      ...
      child: RefreshIndicator(
        ...
        onRefresh: () {
          ...
          Log.ingreso(context, "BAJAR LA PANTALLA, ACTUALIZAR PRERESERVAS");
                      ...
                      child: CustomScrollView(
                        ...
                        slivers: [
                          SliverAppBar(
                            ...
                            title: Text('EMBARQUE DE PASAJEROS CONDUCTOR'),
                            floating: false,
                            elevation: 0,
                            pinned: true,
                            backgroundColor: AppColors.mainBlueColor,
                                    ...
                                    Container(
                                      child: Row(
                                        children: [
                                          ...
                                          Text(
                                            _viaje.cantReservados.toString(),
                                            style: TextStyle(fontSize: 18, color: AppColors.lightBlue, fontWeight: FontWeight.bold),
                                          ),
                                        ],
                                      ),
      ...
      child: InputDecorator(
        ...
        child: DropdownButtonHideUnderline(
          child: DropdownButton(
            ...
            onChanged: (value) {
              if (value != '-1') {
                Log.ingreso(context, "SELECCIONO PUNTO DE EMBARQUE ${value}");
    ...
    if (pEmbarque.isNotEmpty) {
      for (PuntoEmbarque p in pEmbarque) {
        if (p.id == _opcionSeleccionadaEmbarqueViaje) {
          ...
          Log.ingreso(context, "CONDUCTOR: PUNTO EMBARQUE ${p.nombre} CERRADO /  ${DateTime.now()} / ${responseSuccess ? "SINCRONIZADO" : "NO SINCRONIZADO"} ");
      ...
      if (_usuario.acciones.firstWhereOrNull((accion) => accion.toUpperCase() == "GESTIONAREMBARQUECONDUCTOR") != null) {
        ...
        Log.ingreso(context, "ENTRO A INICIO ");
      }
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  ...
                  Log.ingreso(context, "CONDUCTOR: ${pasajero.numDoc} REVERTIR EMBARQUE /  ${DateTime.now()}");
  ...
  Future<void> EmbarcarPasajero(Pasajero pasajero) async {
    ...
    pasajero.coordenadas = coordenadas;
    ...
    if (resp != null) {
      ...
      if (decodeData["rpta"] != "0" && decodeData["rpta"] != "500") {
        ...
        Log.ingreso(context, "CONDUCTOR: ${pasajero.numDoc} EMBARCADO /  ${DateTime.now()} / ${decodeData["mensaje"]} ");
    ...
    if (!errorValidacion) {
      Log.ingreso(context, "CONDUCTOR: ${pasajero.numDoc} EMBARCADO /  ${DateTime.now()} / ${responseSuccess ? "SINCRONIZADO" : "NO SINCRONIZADO"} ");
  ...
  Future<void> RevertirPasajero(Pasajero pasajero) async {
    ...
    pasajero.idEmbarqueReal = _opcionSeleccionadaEmbarqueViaje;
    //pasajero.fechaEmbarque = DateFormat.yMd().add_Hms().format(new DateTime.now());
    pasajero.fechaDesembarque = DateFormat.yMd().add_Hms().format(new DateTime.now());
    ...
    Log.ingreso(context, "CONDUCTOR: ${pasajero.numDoc} REVERTIDO /  ${DateTime.now()} / ${responseSuccess ? "SINCRONIZADO" : "NO SINCRONIZADO"} ");
  ...
  _validarInputDni() async {
    if (_opcionSeleccionadaEmbarqueViaje == '-1') {
      ...
      if (numDocBuscar != "") {
        ...
        Log.ingreso(context, "CONDUCTOR: DNI  ${numDocBuscar} /  ${DateTime.now()}");
        ...
        Log.ingreso(context, "CONDUCTOR: DNI  LIMPIO ${numDocBuscar} /  ${DateTime.now()}");
        ...
        if (viajeActual.pasajeros.isNotEmpty) {
          ...
          if (encontrado == false) {
            Log.ingreso(context, "CONDUCTOR: ${numDocBuscar} NO ENCONTRADO /  ${DateTime.now()}");
            ...
            switch (pasajero.embarcado) {
              case 0:
                if (pasajero.idEmbarque != _opcionSeleccionadaEmbarqueViaje) {
                  Log.ingreso(context, "CONDUCTOR: ${numDocBuscar} NO PUNTO DE EMBARQUE /  ${DateTime.now()}");
                ...
                if (viajeActual.codRuta != pasajero.idRuta) {
                  Log.ingreso(context, "CONDUCTOR: ${numDocBuscar} NO RUTA /  ${DateTime.now()}");
                  return await _mostrarModalPasajeroNoRuta(pasajero);
                }
                if (viajeActual.servicio != pasajero.servicio) {
                  Log.ingreso(context, "CONDUCTOR: ${numDocBuscar} NO SERVICIO /  ${DateTime.now()}");
                ...
                if (fechaHoraSalidaViaje != pasajero.fechaViaje) {
                  Log.ingreso(context, "CONDUCTOR: ${numDocBuscar} NO FECHA O HORA /  ${DateTime.now()}");
                  return await _mostrarModalPasajeroNoFecha(pasajero);
                }
                if (pasajero.idEmbarque == _opcionSeleccionadaEmbarqueViaje) {
                  Log.ingreso(context, "CONDUCTOR: ${numDocBuscar} ENCONTRADO /  ${DateTime.now()}");
                  return await EmbarcarPasajero(pasajero); // JS: 18/07/23 PASAJERO EMBARCADO
                }
                break;
              case 1:
                Log.ingreso(context, "CONDUCTOR: ${numDocBuscar} YA EMBARCADO /  ${DateTime.now()}");
                return _mostrarModalAutomaticoYaEmbarcado(pasajero); //Ya embarcado
              default:
            }
          }
        } else {
          Log.ingreso(context, "CONDUCTOR: ${numDocBuscar} NO HAY PASAJEROS POR EMBARCAR /  ${DateTime.now()}");
          ...
          return WillPopScope(
              child: AlertDialog(
                ...
                actions: [
                  ...
                  TextButton(
                    onPressed: () async {
                      Log.ingreso(context, "CONDUCTOR: ${pasajero.numDoc} EMBARCAR DE TODOS MODOS /  ${DateTime.now()} ");
                      Viaje viajeActual = Provider.of<ViajeProvider>(context, listen: false).viaje;
                      String fechaHoraEmb = DateFormat.yMd().add_Hms().format(new DateTime.now());
  ...
  ActualizarBajarPantalla(String tipoDoc, String numDoc, String codOperacion) async {
    ...
    if (_hayConexion()) //si hay conexion a internet
    {
      ...
      var viajeServicio = new ViajeServicio();
      final viajes = await viajeServicio.obtenerViajesProgramdosBolsaActualizar(tipoDoc, numDoc, codOperacion);
      ...
      if (viajes.isNotEmpty && viajes[0].rpta == "0") {
        ...
        for (var i = 0; i < viajes.length; i++) {
          Viaje viajeExist = Viaje.fromJsonMapVinculadoLocal(listaViajesBolsa.firstWhere((element) => element["nroViaje"] == viajes[i].nroViaje));
          ...
          final listadoPrereservas = await servicio.obtener_nuevos_pasajeros(
            ...
            codOperacion: viajes[i].subOperacionId,
          );
        ...
        Navigator.pop(context, 'Cancel');
        _mostrarModalRespuesta("Error", "El viaje paso su fecha de embarque", false).show();
      }
    } else {
      Navigator.pop(context, 'Cancel');
