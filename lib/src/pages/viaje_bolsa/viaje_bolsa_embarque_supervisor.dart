...
import 'package:embarques_tdp/src/utils/geo_manager.dart'; // Ajusta la ruta

import '../../models/pasajero.dart';
import '../../models/usuario.dart';

class ViajeBolsaEmbarquePage_Supervisor extends StatefulWidget {
  const ViajeBolsaEmbarquePage_Supervisor({Key? key}) : super(key: key);

  @override
  State<ViajeBolsaEmbarquePage_Supervisor> createState() => _ViajeBolsaEmbarquePage_SupervisorState();
}

class _ViajeBolsaEmbarquePage_SupervisorState extends State<ViajeBolsaEmbarquePage_Supervisor> {
  ...
  void initState() {
    ...
    geo.iniciar(
      onActualizar: (pos) {
        ...
        // Aquí podrías actualizar el backend, provider o variable global
      },
      accuracy: AppData.accuracy, // mayor consumo
      distanceFilter: AppData.radioGeocerca, // 🔹 solo si se mueve 2 metros o más
    );
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  ...
                  await CambiarEstadoPasajero(pasajero, embarcar: false);
  ...
  //   final viaje = viajeProvider.viaje;

  //   // 📍 Obtener coordenadas (rápido y con fallback)
  //   String coordenadas;
  //   try {
  //     final pos = await Geolocator.getCurrentPosition();
  //     coordenadas = "${pos.latitude},${pos.longitude}";
  //   } catch (_) {
  //     coordenadas = "0,0"; // fallback seguro
  //   }

  //   // 📦 Preparar pasajero actualizado
  //   final pasajeroActualizado = pasajero
  //     ..embarcado = 1
  //     ..idEmbarqueReal = _opcionSeleccionadaEmbarqueViaje
  //     ..fechaEmbarque = DateFormat.yMd().add_Hms().format(DateTime.now())
  //     ..estado = 'A'
  //     ..coordenadas = coordenadas
  //     ..embarcadoPor = CodigoExternoOdni ? "COD" : "NDI";

  //   // 💾 Guardar SIEMPRE en base local
  //   await AppDatabase.instance.Update(
  //     table: "pasajero",
  //     value: {
  //       "estado": "A",
  ...
  //       "sincronizar": "1", // Siempre marcamos pendiente al inicio
  //       "embarcadoPor": pasajeroActualizado.embarcadoPor,
  //       "coordenadas": pasajeroActualizado.coordenadas,
  //       "fechaEmbarque": pasajeroActualizado.fechaEmbarque,
  //     },
  //     where: "numDoc='${pasajero.numDoc}' AND idRuta='${pasajero.idRuta}'",
  //   );
  ...
  //   await AppDatabase.instance.Update(
  //     table: "usuario",
  //     value: {"sesionSincronizada": "1"},
  //     where: "numDoc='${usuario.numDoc}'",
  //   );

  //   // 🔄 Actualizar el provider en memoria
  //   await viajeProvider.embarcarPasajero(pasajeroActualizado);

  //   bool sincronizado = false;

  //   // 🌐 Intentar sincronizar con backend (si hay conexión)
  //   // if (_hayConexion()) {
  //   //   setState(() => _mostrarCarga = true);

  //   //   try {
  //   //     final servicio = PasajeroServicio();
  //   //     final resp = await servicio
  //   //         .cambiarEstadoPrereservaV5(
  //   //           pasajeroActualizado,
  //   //           viaje.codOperacion,
  //   //           viaje.nroViaje,
  ...
  //   //         await AppDatabase.instance.Update(
  //   //           table: "usuario",
  //   //           value: {"sesionSincronizada": "0"},
  //   //           where: "numDoc='${usuario.numDoc}'",
  //   //         );
  //   //       }
  //   //     }
  //   //     setState(() => _mostrarCarga = false);
  //   //   } catch (_) {
  //   //     setState(() => _mostrarCarga = false);
  //   //     // Si falla, quedará marcado como "pendiente"
  //   //   }
  //   // }

  //   // 🪩 Mostrar modal automático
  //   _modalAutomatico('0', pasajeroActualizado);

  //   // 🧾 Log (para seguimiento interno)
  //   Log.ingreso(
  //     context,
  //     "SUPERVISOR: ${pasajero.numDoc} EMBARCADO / ${DateTime.now()} / "
  //     "${sincronizado ? "SINCRONIZADO" : "NO SINCRONIZADO"}",
  //   );
  // }

  /// 🔁 Reintento manual o automático de sincronización
  Future<void> _sincronizarBackend(Pasajero pasajero, Viaje viaje, Usuario usuario) async {
    try {
      final servicio = PasajeroServicio();
      final resp = await servicio
          .cambiarEstadoPrereservaV5(
      ...
      // Ignoramos si hay error de red o timeout
    }
  }
  ...
  Future<void> CambiarEstadoPasajero(Pasajero pasajero, {required bool embarcar}) async {
    final viajeProvider = Provider.of<ViajeProvider>(context, listen: false);
    final viaje = viajeProvider.viaje;
    final usuario = _usuario; // asumiendo que lo tienes cargado
    final pasajeroServicio = PasajeroServicio();

    setState(() => _mostrarCarga = true);

    // 📍 Intentar obtener coordenadas solo si embarcamos
    String coordenadas = "0,0";
    if (embarcar) {
      // try {
      // final pos = await Geolocator.getCurrentPosition();
      // print({"coordenada Geolocation", pos});
      coordenadas = "${_posicionActual!.latitude},${_posicionActual!.longitude}";
      // coordenadas = "${pos.latitude},${pos.longitude}";
      // print({"coordenada Location", coordenadas});
    }
    // 🧠 Configurar datos según acción
    pasajero
      ..idEmbarqueReal = embarcar ? _opcionSeleccionadaEmbarqueViaje : '0'
      ..fechaEmbarque = DateFormat.yMd().add_Hms().format(DateTime.now())
      ..embarcado = embarcar ? 1 : 0
      ..estado = embarcar ? 'A' : 'P'
      ..nroViaje = embarcar ? viaje.nroViaje : "0"
      ..embarcadoPor = embarcar ? (CodigoExternoOdni ? "COD" : "NDI") : null
      ..coordenadas = embarcar ? coordenadas : null;

    bool sincronizado = false;
    bool errorValidacion = false;

    // 🌐 Intento de sincronización con backend (no bloquea flujo)
    try {
      final resp = await pasajeroServicio.cambiarEstadoPrereservaV5(
        pasajero,
        viaje.codOperacion,
        embarcar ? viaje.nroViaje : "0",
        "${usuario.tipoDoc}${usuario.numDoc}",
        viaje.nroViaje,
      );
      ...
      if (resp != null) {
        ...
        if (data["rpta"] == "0") {
          sincronizado = true;
        } else if (data["rpta"] != "500") {
          errorValidacion = true;
          setState(() => _mostrarCarga = false);
          Log.ingreso(
            context,
            "SUPERVISOR: ${pasajero.numDoc} ${embarcar ? "EMBARCADO" : "REVERTIDO"} / ${DateTime.now()} / ${data["mensaje"]}",
          );
          return _mostrarModalRespuesta("Error", data["mensaje"], false).show();
        }
      }
    } catch (_) {
      // ignoramos timeout o red caída
    }

    // 💾 Guardado local inmediato (sin importar conexión)
    await AppDatabase.instance.Update(
      table: "pasajero",
      value: {
        "estado": pasajero.estado,
        "idEmbarqueReal": pasajero.idEmbarqueReal,
        "embarcado": pasajero.embarcado,
        "nroViaje": pasajero.nroViaje,
        "sincronizar": sincronizado ? "0" : "1",
        "embarcadoPor": pasajero.embarcadoPor ?? "",
        "coordenadas": pasajero.coordenadas ?? "",
        "fechaEmbarque": pasajero.fechaEmbarque,
        if (!embarcar) "fechaDesembarque": pasajero.fechaEmbarque,
      },
      where: "numDoc='${pasajero.numDoc}' AND idRuta='${pasajero.idRuta}'",
    );
    ...
    await AppDatabase.instance.Update(
      table: "usuario",
      value: {"sesionSincronizada": sincronizado ? "0" : "1"},
      where: "numDoc='${usuario.numDoc}'",
    );

    // 🔄 Actualizar estado en Provider
    if (embarcar) {
      await viajeProvider.embarcarPasajero(pasajero);
    } else {
      await viajeProvider.desembarcarPasajero(pasajero);
    }
    ...
    Log.ingreso(
      context,
      "SUPERVISOR: ${pasajero.numDoc} ${embarcar ? "EMBARCADO" : "REVERTIDO"} / ${DateTime.now()} / ${sincronizado ? "SINCRONIZADO" : "NO SINCRONIZADO"}",
    );

    if (embarcar) _modalAutomatico('0', pasajero);
  }
            ...
            switch (pasajero.embarcado) {
              case 0:
                ...
                if (pasajero.idEmbarque == _opcionSeleccionadaEmbarqueViaje) {
                  ...
                  return await CambiarEstadoPasajero(pasajero, embarcar: true);
                }
                break;
              case 1:
                ...
                actions: [
                  ...
                  TextButton(
                    onPressed: () async {
                      ...
                      await CambiarEstadoPasajero(pasajero, embarcar: true);
                      // _modalAutomatico('0', pasajero);
                    },
                    child: Text(
