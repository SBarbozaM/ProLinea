...
class DetalleOrdenServicioMantenimientoPage extends StatefulWidget {
  final OsOrden orden;

  const DetalleOrdenServicioMantenimientoPage({Key? key, required this.orden}) : super(key: key);

  @override
  State<DetalleOrdenServicioMantenimientoPage> createState() => _DetalleOrdenServicioMantenimientoPageState();
}

class _DetalleOrdenServicioMantenimientoPageState extends State<DetalleOrdenServicioMantenimientoPage> {
  ...
  ListarRequerimientos() async {
    ...
    OsRequerimientosUnidad requeResponse = await sOrden.BuscarProblemasUnidad_Mantenimiento(
      CodVeh: widget.orden.codVeh,
      CodPro: widget.orden.codPro.toString(),
      Tdoc: usuario.tipoDoc,
      Ndoc: usuario.numDoc,
    );

    if (requeResponse.rpta == '0') {
      // List<OsRequerimientos> listaReqUsuario = [];
      // var listaRequerimientosUsuario = requeResponse.lista.map((element) {
      //   if(ele){}
      // });
  ...
  ListarTrabajos(OsRequerimientos orden) async {
    ...
    OsTrabajosUnidad trabaResponse = await sOrden.Listar_BuscarTrabajosRegistrados_Mantenimiento(
      NroOS: widget.orden.nro.toString(),
      taller: widget.orden.codTaller,
      CodPro: orden.pro.toString(),
      Tdoc: usuario.tipoDoc,
      Ndoc: usuario.numDoc,
    );
